...
# from .cache import myCache as ttlache
from .rediscache import myCache as rediscache
import os
...
class AuthManagerRedis(BaseManager):
	...
	__type = None
	__table = {
		"itemTable": os.getenv("RBAC_AUTH_ITEM","auth_item"),
		"itemChildTable": os.getenv("RBAC_AUTH_ITEM_CHILD","auth_item_child"),
		"assignmentTable": os.getenv("RBAC_AUTH_ASSIGNMENT","auth_assignment"),
		"ruleTable": os.getenv("RBAC_AUTH_RULE","auth_rule"),
		"groupTable": os.getenv("RBAC_AUTH_GROUP","auth_group")
	}
	cache = None
	c = None
	cacheKey = "AUTHRBACNAKULAREDIS"
	item = None
	items = {}
	rules = {}
	parents = {}
	...
	def __init__(self, db=None):
		...
		print("USE CACHE", os.getenv("CACHE"))
		# if db == None:
		# 	self.db = Request.app.state.db;
		self.db = db
		self.usecache = os.getenv("CACHE","ttl")
		self.c = rediscache()
		# if self.c.get(self.cacheKey) == None:
		# 	self.c[self.cacheKey] = {}
		
	@staticmethod
	async def __merge_two_dicts(x, y):
		z = x.copy()
		z.update(y)
		return z
	
	async def checkAccess(self, userId, permissionName, params={}):
	...
	async def __checkAccessFromCache(self, user, itemName, params, assignments):
	...
	async def __checkAccessRecursive(self, user, itemName, params, assignments):
		item = await self.__getItem(itemName)
		if item == None:
			return False
		if self.executeRule(user, item, params) == False:
			return False
	...
	async def __getItem(self, name=None):
		name = name.strip()
		if name == None:
			return None
	...
	async def __getItems(self, itemType):
		cur = self.db.getDb()
	...
	async def __getChildrenList(self):
		cur = self.db.getDb()
	...
	async def __getChildrenRecursive(self,name,childrenlist,result):
		if name in childrenlist:
			for child in childrenlist[name]:
				result[child] = True
				self.__getChildrenRecursive(child,childrenlist,result)
	
	async def __addItem(self, item):
		stime = int(time.time())
		cur = self.db.getDb()
		# print(item.group)
		if hasattr(item, "group") and isinstance(item.group, AuthGroup):
			item.group = item.group.name
		
		if hasattr(item, "rule") and issubclass(type(item.rule), AuthRule):
	...
	async def __addGroup(self, object):
		group = object
		stime = int(time.time())
		cur = self.db.getDb()
	...
	async def __removeItem(self, item):
		cur = self.db.getDb()
		qp = {"name": item.name}
		# q="""
	...
	async def __removeRule(self, rule):
		cur = self.db.getDb()
	...
	async def __removeGroup(self, group):
		cur = self.db.getDb()
		qp = {"group": group.name}
	...
	async def __updateItem(self, name, item):
	...
	async def __updateRule(self, name, rule):
	...
	async def __updateGroup(self, name, group):
		stime = int(time.time())
		group.update = stime
		cur = self.db.getDb()
		q = """
	...
	async def add(self, object):
		if isinstance(object, AuthGroup):
			await self.__addGroup(object)
		elif isinstance(object, AuthPermission):
			await self.__addItem(object)
		elif isinstance(object, AuthRole):
			await self.__addItem(object)
		elif issubclass(type(object), AuthRule):
			# print("add Rule")
			await self.__addRule(object)

	async def remove(self, object):
		if isinstance(object, AuthGroup):
			await self.__removeGroup(object)
		elif isinstance(object, AuthPermission):
			await self.__removeItem(object)
		elif isinstance(object, AuthRole):
			await self.__removeItem(object)
		elif isinstance(object, AautRule):
			await self.__removeRule(object)

	async def update(self, name, object):
		# print("UPDATE ITEM Parent Called")
		if isinstance(object, AuthGroup):
			await self.__updateGroup(name, object)
		elif isinstance(object, AuthPermission):
			await self.__updateItem(name, object)
		elif isinstance(object, AuthRole):
			await self.__updateItem(name, object)
		elif issubclass(type(object), AuthRule):
			await self.__updateRule(name, object)

	async def getRole(self, name):
		item = await self.__getItem(name)
		# , item.type == self.TYPE_ROLE
		await self.loadFromCache()
		# print("ITEM TYPE", type(item), name, issubclass(type(item), Auth), item.type == self.TYPE_ROLE)
		if issubclass(type(item), Auth) and item.type == self.TYPE_ROLE:
			return item
		else:
			return None

	async def getPermission(self, name):
		item = await self.__getItem(name)
	...
	async def getAssignment(self,roleName,userId=None):
		if userId == None:
			return None
		cur = self.db.getDb()
		q=""" 
	...
	async def getRoles(self):
		return await self.__getItems(self.TYPE_ROLE)

	async def getDefaultRoleInstances(self):
		pass

	async def getPermissions(self):
		return await self.__getItems(self.TYPE_PERMISSION)

	async def getPermissionsGrouped(self):
		cur = self.db.getDb()
		q=""" 
		...
		for row in res:
			...
			if row["group"] not in permissions:
				...
				permissions[row["group"]]["group"] = await self.getGroup(row["group"])
	...
	async def getAssignments(self,userId = None):
		if userId == None:
			return {}
		cur = self.db.getDb()
		q="""
	...
	async def executeRule(self, user, item,  params):
	...
	async def getRolesByUser(self,userId = None):
		cur = self.db.getDb()
		# userId = userId.strip()
		if userId == None:
			return {}
	...
	async def getChildRoles(self, roleName):
	...
	async def getPermissionsByRole(self, roleName):
	...
	async def getPermissionsByUser(self, userId = None):
		if userId == None:
			return {}
		cur = self.db.getDb()
		q="""
	...
	async def canAddChild(self,parent,child):
		return not self.__detectLoop(parent,child)

	async def addChild(self, parent, child):
		# try:
		...
		cur = self.db.getDb()
		cur.execute(q, qp)
		print("adding_child",q,qp, "cur", cur)
		await self.invalidateCache()
		return True

	async def removeChild(self,parent,child):
		cur = self.db.getDb()
		q = """
	...
	async def removeChildren(self, parent):
		cur = self.db.getDb()
		q = """
	...
	async def revoke(self, role=None, userId=None):
		if userId == None:
			return False
	...
	async def revokeAll(self, userId=None):
		if userId == None:
			return False
			# raise ValueError("USerid and RoleName Must Be Given, 0 Given")
		self.__checkAccessAssignments.pop(userId, None)
	...
	async def __removeAllItems(self, utype):
	...
	async def removeAllAssignments(self):
	...
	async def loadFromCache(self):
		print("LOAD FROM CACHE")
		if await self.c.get(self.cacheKey) == None:
			# await self.c[self.cacheKey] = {}
			await self.c.set(self.cacheKey,{})

		if self.items != None:
			return True

		data = await self.c.get(self.cacheKey)
		...
		await self.c.set(self.cacheKey,tmpCache)
	...
	async def invalidateCache(self):
		if await self.c.has(self.cacheKey):
			await self.c.set(self.cacheKey, {})
			self.items = {}
			self.rules = {}
			self.parents = {}
